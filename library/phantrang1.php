	...
	if($totalPage > 1){
		...
		$actual_link = (isset($_SERVER['HTTPS']) ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
			...
			//page đầu, page trước
				...
				$prew="";
				if($currentPage>1){
					$first= "<a href='$actual_link&page=1#comment-box'>First</a>";
					$prew= "<a href='$actual_link&page=".($currentPage-1)."#comment-box'><</a>";
				}
			//page tiếp theo, page cuối
				$next="";
				$last="";
				if($currentPage<$totalPage){
					$last= "<a href='$actual_link&page=$totalPage#comment-box'>Last</a>";
					$next= "<a href='$actual_link&page=".($currentPage+1)."#comment-box'>></a>";
				}	
					...
					for($i=$startPage;$i<=$endPage;$i++){
						if($i==$currentPage){
							$listPage.="<a href='$actual_link&page=$i#comment-box' class='active'>$i</a>";
						}else{
							$listPage.="<a href='$actual_link&page=$i#comment-box'>$i</a>";
						}
					}
					//hiện thị
...
function phantrangcate($pageShow,$displayItem,$totalPage,$start)
{
	if($totalPage > 1){
		//==========lấy đường dẫn hiên tại//// không thể dùng cho nhiều ?cid=11?page=13
		...
		//$actual_link = (isset($_SERVER['HTTPS']) ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[PHP_SELF]";
			...
			//-------Tìm actual link bản gốc, bị hạn chế nếu 100 page rở lên---------
